...
async function getPlaylists(playlists) {
    ...
    if (config.topPlaylist.enabled) {
        getCatList().then(async x => {
            if (!config.topPlaylist.category in x) {
            ...
            let result = await rp(
                options(
                    `${server}top/playlist?limit=${c.limit}&order=${c.order in ['hot', 'new'] ? c.order : 'hot'}&cat=${
						c.category
					}`
                )
            );
    ...
    if (config.hqPlaylist.enabled) {
        getCatList().then(async x => {
            if (!config.hqPlaylist.category in x) {
                console.error(`[DataModules][Netease2] topPlaylist 的分類出錯，已預設為 ACG`);
                config.hqPlaylist.category = 'ACG';
            }
            let c = config.hqPlaylist;
            let result = await rp(options(`${server}top/playlist/highquality?limit=${c.limit}&cat=${c.category}`));
            if (result.code == 200) {
                r = r.concat(
                    result.playlists.map(k => ({
                        name: k.name,
