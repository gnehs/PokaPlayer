...
fs.readdir(__dirname + "/dataModule", (err, files) => {
    ...
    files.forEach(async file => {
        if (path.extname(file) == ".js") {
            ...
            let enabled =
                moduleData.active.indexOf("onLoaded") > -1 && config ? await _module.onLoaded() : true;
...
if (!config) {
    router.post("/testconnection", async(req, res) => {
        ...
        console.log(req.body.moduleName)
            // 沒這東西
        if (!_module || moduleList[moduleName].active.indexOf("testConnection") == -1)
            return res.send("disabled");
        try {
            res.send(await _module.testConnection(req.body.configData));
        } catch (e) {
...
router.use((req, res, next) => {
    if (!config)
        res.status(200).send("No config.json");
