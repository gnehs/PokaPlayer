...
function setLrc(lrcResult) {
    if (lrcResult) lrc.load(lrcResult);
    else lrc.load(`[00:00.000]無歌詞`);
...
ap.on("timeupdate", () => {
    ...
    let currentTime = ap.audio.currentTime ? secondToTime(ap.audio.currentTime) : "0:00",
        ...
        timelineBufferedColor = $("body").hasClass("mdui-theme-layout-dark")
...
async function showSearch(keyword) {
    ...
    let noResult = `<div class="mdui-valign" style="height:150px"><p class="mdui-center">${
...
async function showPlaylist(data) {
    ...
    mdui.mutation();
    let result = data || (await axios.get(`/pokaapi/playlists`));
    if ($("#content").attr("data-page") == "playlist") {
        if ((data ? result.playlists.length : result.data.playlists.length) < 0)
            $("#content").html(
...
function getSnackbarPosition() {
    if ($(window).width() < 768) return "left-top";
